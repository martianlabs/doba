...
DOBA_TEST("values are distinct and begin with unknown") {
  ...
  DOBA_EXPECT(target::kOriginForm != target::kUnknown);
  DOBA_EXPECT(target::kAbsoluteForm != target::kOriginForm);
  DOBA_EXPECT(target::kAuthorityForm != target::kAbsoluteForm);
  DOBA_EXPECT(target::kAsteriskForm != target::kAuthorityForm);
}
