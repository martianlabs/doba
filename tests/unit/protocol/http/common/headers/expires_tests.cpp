...
namespace {
using martianlabs::doba::protocol::http::headers::expires;
}  // namespace
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!expires::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!expires::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!expires::check(std::string_view{"\x80", 1}));
...
DOBA_TEST("check rejects incomplete and corrupted date fields") {
  ...
  for (const auto seed : seeds) {
    for (std::size_t position = 0; position < seed.size(); ++position) {
      martianlabs::doba::tests::unit::test_helper::set_context(
          std::string(seed) + ", position " + std::to_string(position));
      DOBA_EXPECT(!expires::check(seed.substr(0, position)));
      std::string source(seed);
      source[position] = '!';
      DOBA_EXPECT(!expires::check(source));
    }
