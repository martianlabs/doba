...
namespace {
using martianlabs::doba::protocol::http::headers::vary;
}  // namespace
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!vary::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!vary::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!vary::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "Accept-Encoding";
