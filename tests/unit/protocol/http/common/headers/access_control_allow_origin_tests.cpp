...
namespace {
using martianlabs::doba::protocol::http::headers::access_control_allow_origin;
}  // namespace
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!access_control_allow_origin::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!access_control_allow_origin::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!access_control_allow_origin::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "https://example.com";
