...
namespace {
using martianlabs::doba::protocol::http::headers::cookie;
}  // namespace
...
DOBA_TEST("check accepts valid values") {
  constexpr std::string_view cases[] = {
      "a=b",     "a=",       "a=abc123",     "a=!#$%&'()*+-./:<=>?@[]^_`{|}~",
      "a=\"b\"", "a=b; c=d", "a=b; c=\"d\"", "__Host-id=123",
  };
...
DOBA_TEST("check rejects invalid values") {
  constexpr std::string_view cases[] = {
      "",          " ",         "=b",    "a",        "a b=c",
      "a=b;c=d",   "a=b;  c=d", "a=b; ", "a=b, c=d", "a=\"unterminated",
      "a=\"b c\"", "a=b\r",
  };
  for (const auto source : cases) {
    DOBA_EXPECT(!cookie::check(source));
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!cookie::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!cookie::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!cookie::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "a=b";
  ...
  padded.push_back('\0');
  padded += "suffix";
  DOBA_EXPECT(cookie::check(std::string_view(padded.data(), seed.size())));
