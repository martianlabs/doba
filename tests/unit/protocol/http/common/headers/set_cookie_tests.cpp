...
namespace {
using martianlabs::doba::protocol::http::headers::set_cookie;
}  // namespace
...
DOBA_TEST("check rejects invalid values") {
  constexpr std::string_view cases[] = {
      "",      " ",    "=b",           "a",
      "a b=c", "a=b;", "a=b;; Secure", "a=\"unterminated",
      "a=b\r",
  };
  for (const auto source : cases) {
    DOBA_EXPECT(!set_cookie::check(source));
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!set_cookie::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!set_cookie::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!set_cookie::check(std::string_view{"\x80", 1}));
