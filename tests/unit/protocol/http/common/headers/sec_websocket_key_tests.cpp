...
namespace {
using martianlabs::doba::protocol::http::headers::sec_websocket_key;
}  // namespace
...
DOBA_TEST("check handles string view boundaries") {
  DOBA_EXPECT(!sec_websocket_key::check(std::string_view{"\0", 1}));
  DOBA_EXPECT(!sec_websocket_key::check(std::string_view{"a\0", 2}));
  DOBA_EXPECT(!sec_websocket_key::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!sec_websocket_key::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!sec_websocket_key::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "dGhlIHNhbXBsZSBub25jZQ==";
