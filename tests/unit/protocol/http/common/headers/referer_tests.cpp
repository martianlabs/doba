...
namespace {
using martianlabs::doba::protocol::http::headers::referer;
}  // namespace
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!referer::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!referer::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!referer::check(std::string_view{"\x80", 1}));
