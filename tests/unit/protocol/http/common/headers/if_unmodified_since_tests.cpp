...
namespace {
using martianlabs::doba::protocol::http::headers::if_unmodified_since;
}  // namespace
...
DOBA_TEST("check handles string view boundaries") {
  DOBA_EXPECT(!if_unmodified_since::check(std::string_view{"\0", 1}));
  DOBA_EXPECT(!if_unmodified_since::check(std::string_view{"a\0", 2}));
  DOBA_EXPECT(!if_unmodified_since::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!if_unmodified_since::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!if_unmodified_since::check(std::string_view{"\x80", 1}));
