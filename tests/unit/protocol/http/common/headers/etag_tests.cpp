...
namespace {
using martianlabs::doba::protocol::http::headers::etag;
}  // namespace
...
DOBA_TEST("check rejects invalid values") {
  constexpr std::string_view cases[] = {
      "",          " ",          "abc",        "W/abc",
      "w/\"abc\"", "W /\"abc\"", "W/ \"abc\"", "\"unterminated",
      "\"a b\"",   "\"a\\\"b\"", "\"a\r\"",    "\"a\n\"",
  };
  for (const auto source : cases) {
    DOBA_EXPECT(!etag::check(source));
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!etag::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!etag::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!etag::check(std::string_view{"\x80", 1}));
  ...
  padded.push_back('\0');
  padded += "suffix";
  DOBA_EXPECT(etag::check(std::string_view(padded.data(), seed.size())));
