...
namespace {
using martianlabs::doba::protocol::http::headers::content_encoding;
}  // namespace
...
DOBA_TEST("check accepts valid values") {
  constexpr std::string_view cases[] = {
      "", ",", "gzip", "identity", "x-custom", "gzip, br", ", gzip, , br,",
  };
  for (const auto source : cases) {
    DOBA_EXPECT(content_encoding::check(source));
    const std::string padded = "x" + std::string(source) + "y";
    DOBA_EXPECT(content_encoding::check(
...
DOBA_TEST("check rejects invalid values") {
  constexpr std::string_view cases[] = {
      " ", "g zip", "\"gzip\"", "gzip:", "gzip/br", "gzip;br", "gzip,\nbr",
  };
  for (const auto source : cases) {
    DOBA_EXPECT(!content_encoding::check(source));
  }
...
DOBA_TEST("check handles string view boundaries") {
  DOBA_EXPECT(!content_encoding::check(std::string_view{"\0", 1}));
  DOBA_EXPECT(!content_encoding::check(std::string_view{"a\0", 2}));
  DOBA_EXPECT(!content_encoding::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!content_encoding::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!content_encoding::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "gzip";
...
DOBA_TEST("check validates every byte inside a token") {
  ...
  for (unsigned int byte = 0; byte <= 255; ++byte) {
    ...
    DOBA_EXPECT_EQUAL(content_encoding::check(source), expected);
  }
}
