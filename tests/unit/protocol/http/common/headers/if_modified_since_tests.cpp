...
namespace {
using martianlabs::doba::protocol::http::headers::if_modified_since;
}  // namespace
...
DOBA_TEST("check accepts valid values") {
  ...
  for (const auto source : cases) {
    DOBA_EXPECT(if_modified_since::check(source));
    const std::string padded = "x" + std::string(source) + "y";
    DOBA_EXPECT(if_modified_since::check(
...
DOBA_TEST("check handles string view boundaries") {
  DOBA_EXPECT(!if_modified_since::check(std::string_view{"\0", 1}));
  DOBA_EXPECT(!if_modified_since::check(std::string_view{"a\0", 2}));
  DOBA_EXPECT(!if_modified_since::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!if_modified_since::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!if_modified_since::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "Sun, 06 Nov 1994 08:49:37 GMT";
...
DOBA_TEST("check rejects incomplete and corrupted date fields") {
  ...
  for (const auto seed : seeds) {
    for (std::size_t position = 0; position < seed.size(); ++position) {
      martianlabs::doba::tests::unit::test_helper::set_context(
          std::string(seed) + ", position " + std::to_string(position));
      DOBA_EXPECT(!if_modified_since::check(seed.substr(0, position)));
      std::string source(seed);
      source[position] = '!';
      DOBA_EXPECT(!if_modified_since::check(source));
    }
