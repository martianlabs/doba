...
namespace {
using martianlabs::doba::protocol::http::headers::content_range;
}  // namespace
...
DOBA_TEST("check accepts valid values") {
  constexpr std::string_view cases[] = {
      "bytes 0-0/1",  "bytes 0-499/1234", "bytes 500-999/*",
      "bytes */1234", "items 1-2/10",     "x 000-001/002",
  };
...
DOBA_TEST("check handles string view boundaries") {
  DOBA_EXPECT(!content_range::check(std::string_view{"\0", 1}));
  DOBA_EXPECT(!content_range::check(std::string_view{"a\0", 2}));
  DOBA_EXPECT(!content_range::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!content_range::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!content_range::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "bytes 0-0/1";
