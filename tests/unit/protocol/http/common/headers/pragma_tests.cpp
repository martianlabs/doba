...
namespace {
using martianlabs::doba::protocol::http::headers::pragma;
}  // namespace
...
DOBA_TEST("check accepts valid values") {
  constexpr std::string_view cases[] = {
      "no-cache",          "foo",
      "foo=bar",           "foo=\"a,b\"",
      "no-cache, foo=bar", ", no-cache, , foo=bar,",
  };
...
DOBA_TEST("check rejects invalid values") {
  constexpr std::string_view cases[] = {
      "",     " ",        ",",        ", ,",     "=bar",
      "foo=", "foo =bar", "foo= bar", "foo;bar", "foo=\"unterminated",
  };
  for (const auto source : cases) {
    DOBA_EXPECT(!pragma::check(source));
  }
...
DOBA_TEST("check handles string view boundaries") {
  ...
  DOBA_EXPECT(!pragma::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!pragma::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!pragma::check(std::string_view{"\x80", 1}));
  constexpr std::string_view seed = "no-cache";
