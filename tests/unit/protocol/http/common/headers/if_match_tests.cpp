...
namespace {
using martianlabs::doba::protocol::http::headers::if_match;
}  // namespace
...
DOBA_TEST("check accepts valid values") {
  ...
  for (const auto source : cases) {
    ...
    DOBA_EXPECT(
        if_match::check(std::string_view(padded).substr(1, source.size())));
  }
}
// +===========================================================================+
...
DOBA_TEST("check handles string view boundaries") {
  DOBA_EXPECT(!if_match::check(std::string_view{"\0", 1}));
  DOBA_EXPECT(!if_match::check(std::string_view{"a\0", 2}));
  DOBA_EXPECT(!if_match::check(std::string_view{"a\r", 2}));
  DOBA_EXPECT(!if_match::check(std::string_view{"a\n", 2}));
  DOBA_EXPECT(!if_match::check(std::string_view{"\x80", 1}));
