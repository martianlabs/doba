...
class wake_resource {
 public:
  ...
  // | [>] METHODs                                                  ( public ) |
  // +=========================================================================+
  wake_resource() : handle_(eventfd(0, EFD_NONBLOCK | EFD_CLOEXEC)) {}
  ~wake_resource() {
...
DOBA_TEST("dispatcher wakes and resumes queued work once") {
  ...
  DOBA_EXPECT_EQUAL(count.load(), 0);
  uint64_t wake = 0;
  DOBA_EXPECT_EQUAL(::read(resource.get(), &wake, sizeof(wake)), sizeof(wake));
