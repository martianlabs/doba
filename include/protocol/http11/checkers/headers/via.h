...
#ifndef martianlabs_doba_protocol_http11_checkers_h_via_h
#define martianlabs_doba_protocol_http11_checkers_h_via_h
...
// |                                                                           |
// | Via can be used for tracking message forwards, avoiding request loops,    |
// | and identifying the protocol capabilities of senders along the message    |
// | forwarding chain.                                                         |
// |                                                                           |
...
// | about how the message was received, so the final field value is ordered   |
// | according to the sequence of forwarding recipients.                       |
// |                                                                           |
// | A proxy MUST send an appropriate Via header field in each message that it |
// | forwards. An HTTP-to-HTTP gateway MUST send an appropriate Via header     |
// | field in each inbound request message and MAY send a Via header field in  |
// | forwarded response messages.                                              |
// |                                                                           |
// | For each intermediary, received-protocol identifies the protocol and      |
// | protocol version used by the upstream sender of the message. For brevity, |
...
// | the recipient server or client that subsequently forwarded the message.   |
// | If the real host is sensitive information, a sender MAY replace it with a |
// | pseudonym. If no port is provided, a recipient MAY interpret that as the  |
// | default port, if any, for the received-protocol.                          |
// |                                                                           |
// | A sender MAY generate comments to identify the software of each recipient.|
// | Comments in Via are optional, and a recipient MAY remove them before      |
...
// | Sender syntax:                                                            |
// |                                                                           |
// |   #via-member = [ via-member *( OWS "," OWS via-member ) ]                |
// |                                                                           |
// | Recipient syntax:                                                         |
// |                                                                           |
// |   #via-member = [ via-member ] *( OWS "," OWS [ via-member ] )            |
...
// |                                                                           |
// | Since the rule is "#via-member", rather than "1#via-member", zero         |
// | non-empty via-member elements are permitted by the purely syntactic ABNF. |
// +---------------------------------------------------------------------------+
// | Parser notes                                                              |
// +---------------------------------------------------------------------------+
// | received-by is pseudonym, which is token. Do not parse it as uri-host or  |
// | IP-literal grammar. Dots are allowed because "." is tchar; brackets are   |
// | not allowed by token.                                                     |
// |                                                                           |
// | comment is recursive and can contain nested comments. quoted-pair is only |
// | valid inside comment.                                                     |
// |                                                                           |
...
class via {
 ...
 private:
  ...
  // | via-member = received-protocol RWS received-by [ RWS comment ]          |
  // +-------------------------------------------------------------------------+
  // | When out is non-null, the received-protocol, received-by, and optional  |
  // | comment substrings are captured into it while validation stays          |
  // | identical to the out==nullptr path.                                     |
  // +-------------------------------------------------------------------------+
