...
#ifndef martianlabs_doba_protocol_http11_checkers_h_host_h
#define martianlabs_doba_protocol_http11_checkers_h_host_h
...
// |  Host: [www.example.org:8080](http://www.example.org:8080)                |
// |  Host: [2001:db8::1]:443                                                  |
// +---------------------------------------------------------------------------+
...
// |                | / ";" / "="                                              |
// +---------------------------------------------------------------------------+
// | ABNF literals are case-insensitive unless explicitly defined otherwise;   |
// | therefore, the "v" literal in IPvFuture accepts both "v" and "V".         |
// |                                                                           |
// | Both reg-name and port can be empty because they use the "*" repetition   |
// | operator. Consequently, an empty Host value and an explicitly empty port  |
// | are syntactically valid according to this ABNF.                           |
// |                                                                           |
...
class host {
 ...
 private:
  ...
  // +=========================================================================+
  // | [>] skip_n_hextets                                          ( private ) |
  // +=========================================================================+
  // +----------------+--------------------------------------------------------+
  // | h16            | 1*4HEXDIG                                              |
  // +----------------+--------------------------------------------------------+
  static int skip_n_hextets(std::string_view sv) {
    int hextets_found = 0;
    std::size_t off = 0;
  ...
  // |                | / "25" %x30-35           ; 250-255                     |
  // +----------------+--------------------------------------------------------+
  static bool check_dec_octet(std::string_view sv, std::size_t digits,
                              std::size_t off) {
  ...
  // +=========================================================================+
  // | [>] check_ip_v_future                                       ( private ) |
  // +=========================================================================+
  // +--------------+----------------------------------------------------------+
  // | IPvFuture    | "v" 1*HEXDIG "." 1*( unreserved / sub-delims / ":" )     |
  // +--------------+----------------------------------------------------------+
  static bool check_ip_v_future(std::string_view sv) {
  ...
  // +=========================================================================+
  // | [>] check_ip_v4_address                                     ( private ) |
  // +=========================================================================+
  // +----------------+--------------------------------------------------------+
  // | IPv4address    | dec-octet "." dec-octet "." dec-octet "." dec-octet    |
  // +----------------+--------------------------------------------------------+
  static bool check_ip_v4_address(std::string_view sv) {
    if (sv.empty()) return false;
  ...
  // | h16            | 1*4HEXDIG                                              |
  // +----------------+--------------------------------------------------------+
  static bool check_ip_v6_address(std::string_view sv) {
  ...
  // +=========================================================================+
  // | [>] check_ip_literal                                        ( private ) |
  // +=========================================================================+
  // +----------------+--------------------------------------------------------+
  // | IP-literal     | "[" ( IPv6address / IPvFuture ) "]"                    |
  // +----------------+--------------------------------------------------------+
  static bool check_ip_literal(std::string_view sv) {
  ...
  // +=========================================================================+
  // | [>] check_reg_name                                          ( private ) |
  // +=========================================================================+
  // +----------------+--------------------------------------------------------+
  // | reg-name       | *( unreserved / pct-encoded / sub-delims )             |
  // +----------------+--------------------------------------------------------+
  static bool check_reg_name(std::string_view sv) {
    std::size_t off = 0;
    while (off < sv.size()) {
  ...
  // | port           | *DIGIT                                                 |
  // +----------------+--------------------------------------------------------+
  static bool check_port(std::string_view sv) {
    std::size_t off = 0;
    while (off < sv.size()) {
