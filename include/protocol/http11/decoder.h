    ...
    while (i < sv_size) {
      if (!field_name_decoded) {
        if (sv[i] == '\r') {
          ...
          if (sv[i + 1] != '\n') return deserialization_status::kInvalidSource;
          i += 2;
          // Let's apply the rules to the context and see if we can accept the
          // request or not (early rejection).
          ...
          // Let's set the request values!
          req_->set_method(std::move(method_tmp));
          req_->set_absolute_path(std::move(absolute_path_tmp));
          req_->set_headers(std::move(headers_tmp));
          req_->set_target(std::move(target_tmp));
          if (!query_tmp.empty()) {
            std::vector<query_parameter> query_parameters_tmp;
            ...
            std::size_t qp_count = helpers::split_query_parameters(
              query_tmp, keys_tmp, values_tmp);
            for (std::size_t q = 0; q < qp_count; q++) {
              query_parameters_tmp.emplace_back(std::string(keys_tmp[q]),
                std::string(values_tmp[q]));
            }
            req_->set_query_parameters(std::move(query_parameters_tmp));
          }
          if (ctx.has_host) {
            req_->set_host(ctx.host.host, ctx.host.port, ctx.host.type);
          }
          if (ctx.has_target_authority) {
            req_->set_target_authority(ctx.target_authority.host,
              ctx.target_authority.port,
              ctx.target_authority.type);
          }
          if (connection_tmp.chunked) {
            req_->set_body(body::deserializer<body::decoder_chunked>::chunked());
          }
          else if (ctx.has_content_length && ctx.content_length > 0) {
            req_->set_body(body::deserializer<body::decoder_raw>::raw(
              {}, ctx.content_length));
          }
          if (req_->has_body()) {
          }
      ...
      header this_header{std::string(field_name), std::string(field_value)};
      headers_tmp.emplace_back(std::move(this_header));
      i += 2;
      fn_start = i;
