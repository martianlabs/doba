...
class body_storage {
 public:
  ...
  // | [>] finish                                                  ( public )  |
  // +=========================================================================+
  void finish(std::uint64_t stored) noexcept {
    total_bytes_ = stored;
    if (spilled_) {
      file_.flush();
      if (!file_) {
  ...
  bool fetch(std::byte& b) {
    ...
    b = static_cast<std::byte>(static_cast<unsigned char>(mem_[decode_pos_++]));
    return true;
  }
  // +=========================================================================+
  // | [>] read                                                    ( public )  |
  ...
  // | [>] total_size                                               ( public ) |
  // +-------------------------------------------------------------------------+
  // | total_bytes_ is set authoritatively by finish() in both modes.          |
  // | Before finish() we cannot know the logical body size: a chunked codec   |
  // | stores more wire bytes than logical bytes, so returning                 |
  // | mem_.size() here would expose the wire size and cause response to       |
  // | emit a wrong Content-Length.                                            |
  // | Always return nullopt until the storage is finalised.                   |
  // +=========================================================================+
  ...
  [[nodiscard]] bool exhausted() const noexcept {
    ...
    // body_codec_raw::decode without requiring finish() to be called first.
 ...
 private:
  ...
  std::size_t read_file(char* out, std::size_t want, std::uint64_t& pos) {
    ...
    std::size_t remaining = static_cast<std::size_t>(total_bytes_ - pos);
  ...
  // +-------------------------------------------------------------------------+
  // | Produces a path that is unique across threads and processes by          |
  // | combining a high-resolution timestamp with a process-lifetime counter.  |
  // +=========================================================================+
